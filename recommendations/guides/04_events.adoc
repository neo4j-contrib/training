...
WHERE group.name STARTS WITH "NYC Neo4j" AND event.time < timestamp()
...
MATCH (member)-[:INTERESTED_IN]->()<-[:HAS_TOPIC]-()-[:HOSTED_EVENT]->(futureEvent) WHERE futureEvent.time > timestamp()
