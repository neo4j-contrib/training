= Getting Started with Graph Databases using Neo4j - Part 4 Indexing, Recommendations, Import
:neo4j-version: 3.1.0
:currentyear: 2017
:doctype: book
:toc: left
:toclevels: 1
:experimental:
:linkattrs:
:image: img
:manual: http://neo4j.com/docs/developer-manual/current
:manual-cypher: {manual}/cypher

include::scripts.txt[]

include::60-Minute-Cypher/0119_indexing_and_labels.adoc[]

include::60-Minute-Cypher/0120_aggregation.adoc[]

include::60-Minute-Cypher/0121_lab_who_are_the_five_busiest_actors.adoc[]

include::60-Minute-Cypher/0122_lab_recommendation_engine.adoc[]

include::60-Minute-Cypher/0123_importing_data.adoc[]

include::60-Minute-Cypher/0124_community_resources_and_next_steps.adoc[]

include::60-Minute-Cypher/0124_Quiz4.adoc[]

link:/graphacademy/online-training/getting-started-graph-databases-using-neo4j/intro-course-finish/["Finish the Course here",role="next-section medium button"]

include::scripts-end.txt[]

++++     
<script>
$( document ).ready(function() {
  Intercom('trackEvent','training-intro-view-part4');
});   
</script>
++++
