= Getting Started with Graph Databases using Neo4j - Part 2 Creating and Querying
:neo4j-version: 3.1.0
:currentyear: 2017
:doctype: book
:toc: left
:toclevels: 1
:linkattrs:
:experimental:
:image: img
:manual: http://neo4j.com/docs/developer-manual/current
:manual-cypher: {manual}/cypher

include::scripts.txt[]

include::60-Minute-Cypher/0106_Nodes_and_Properties.adoc[]

include::60-Minute-Cypher/0107_Relationships.adoc[]

include::60-Minute-Cypher/0108_Deleting_Nodes_and_Relationships.adoc[]

include::60-Minute-Cypher/0110_Lab_All_Characters_in_the_Matrix.adoc[]

include::60-Minute-Cypher/0111_Order_Limit_and_Skip.adoc[]

// the :play movies graph only has Movie and Person labels
//include::60-Minute-Cypher/0112_Lab_exploring_an_unknown_graph.adoc[]


include::60-Minute-Cypher/0112_Quiz2.adoc[]

link:/graphacademy/online-training/getting-started-graph-databases-using-neo4j/part-3/[Continue to Part 3: Filters and Paths,role="next-section medium button"]

include::scripts-end.txt[]

++++     
<script>
$( document ).ready(function() {
  Intercom('trackEvent','training-intro-view-part2');
});   
</script>
++++
