...
WHERE group.name CONTAINS 'Neo4j'
  AND NOT (:Member {name: "Your Name"})-[:MEMBER_OF]->(otherGroup)
RETURN otherGroup.name,
       COUNT(topic) AS topicsInCommon,
       COLLECT(topic.name) AS topics
ORDER BY topicsInCommon DESC
...
[source, cypher]
----
MATCH (member:Member {name: "Your Name"})-[:INTERESTED_IN]->(topic), (member)-[:MEMBER_OF]->(group)-[:HAS_TOPIC]->(topic)
...
[source, cypher]
----
MATCH (member:Member {name: "Your Name"})-[:INTERESTED_IN]->(topic),
...
[source, cypher]
----
MATCH (member:Member {name: "Your Name"})-[:MEMBER_OF]->(group)-[:HAS_TOPIC]->(topic)
WITH member, topic, COUNT(*) AS score
MATCH (topic)<-[:HAS_TOPIC]-(otherGroup)
WHERE NOT (member)-[:MEMBER_OF]->(otherGroup)
RETURN otherGroup.name,
...
[source, cypher]
----
MATCH (member:Member {name: "Your Name"})-[:RSVPD {response: "yes"}]->(event:Event)<-[:RSVPD {response: "yes"}]-(otherMember:Member)
...
MATCH (member:Member {name: "Your Name"})-[:RSVPD {response: "yes"}]->(event:Event)<-[:HOSTED_EVENT]-(group)
...
MATCH (m:Member {name:"Your Name"})-[:RSVPD {response:"yes"}]->(event)<-[:HOSTED_EVENT]->()-[:HAS_TOPIC]->(topic)
